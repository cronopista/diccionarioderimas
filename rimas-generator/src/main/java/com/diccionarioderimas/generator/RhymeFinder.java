...
public class RhymeFinder {
	...
	private List<String> silables;

	public RhymeFinder(String word) {

		this.word = word;
		this.silables = new ArrayList<String>();
		calculate();
	}
	...
	private void calculate() {
		...
		for (int i = 1; i < chars.length; i++) {
			...
			if (chars[i] == 'h' || (chars[i] == 'l' && chars[i - 1] == 'l') || (chars[i] == 'u' && chars[i - 1] == 'q')
					|| (chars[i] == 'u' && chars[i - 1] == 'g' && i + 1 < chars.length
							&& (chars[i + 1] == 'e' || chars[i + 1] == 'i'))
		...
		for (int i = 0; i < newLen; i++) {
			...
			if (i + 1 >= newLen) {
				...
				if (types[i] == VOWEL && i > 1 && types[i - 1] == VOWEL && types[i - 2] == VOWEL && soft[i] == HARD
					...
					countSilables++;
				} else if (types[i] == VOWEL && soft[i] == HARD && i > 0 && types[i - 1] == VOWEL
						&& soft[i - 1] == HARD) {
					silables[countSilables] = i;
					countSilables++;
				}
		...
		saveSilables(chars, oldSil, countSilables);
	...
	private void saveSilables(char[] chars, int[] silables, int countSilables) {
		int c = 0;
		...
		for (int i = 0; i < chars.length; i++) {
			...
			if (c < countSilables && silables[c] == i) {
				...
				this.silables.add(buffer.toString());
	...
	public List<String> getSilables() {
		return silables;
	}
