...
public class WebPageWordHandler implements WordHandler {
	...
	public void wordProduced(String oldWord, String newWord, int frequency, String type, int category) {
		...
		cb.append(newWord).append('\t').append(oldWord).append('\t').append(type).append('\t').append(rhyme.getRhyme())
				.append('\t').append(rhyme.countSil()).append('\t').append(rhyme.startsWithVoH()).append('\t')
	...
	public void save() {
		...
		if (!new File("WDDBB").exists())
			new File("WDDBB").mkdir();
