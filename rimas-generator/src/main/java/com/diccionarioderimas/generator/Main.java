...
public class Main extends JPanel implements WindowListener {
	...
	public Main(String[] args, String baseDir, boolean exit) throws Exception {
		...
		setLayout(null);
		add(progressBar);
		progressBar.setBounds(20, 100, 460, progressBar.getPreferredSize().height);
		add(cwordLabel);
		...
		frame.setIconImage(new ImageIcon(this.getClass().getResource("logoSmall.png")).getImage());
		...
		frame.setLocation(new Point((dim.width - 500) / 2, (dim.height - 300) / 2));
		frame.getContentPane().add(this);
		frame.setVisible(true);

		if (!new File(basePath + "resbc.me").exists()) {
			try {
				Utils.copyFile(this.getClass().getResourceAsStream("resbc.txt"), new File(basePath + "resbc.me"));
			} catch (Exception e) {
		...
		if (args == null || args.length == 0 || args[0].equals("rhyme")) {
			...
			deleteDirectory(new File("WDDBB"));
			bh = new WebPageWordHandler(basePath);
		} else if (args[0].equals("frequency")) {
			deleteDirectory(new File("FDDBB"));
			bh = new FrequencyWordHandler(basePath);
		} else if (args[0].equals("json")) {
			deleteDirectory(new File("JSON"));
			bh = new JSONWordHandler(basePath);
		} else if (args[0].equals("SIL")) {
			bh = new SaveSilablesWordHandler();
		}
		// skip comments
		...
		while ((line = in.readLine()) != null) {
			if (line.length() > 0) {
				...
				if (split[1].indexOf('T') != -1 || split[1].indexOf('I') != -1 || split[1].indexOf('R') != -1) {
					Verb m = new Verb(split[0], split[1], bh, Integer.parseInt(split[2]));
					m.conjugate();
				}else {
					bh.wordProduced(split[0], split[0], Integer.parseInt(split[2]), "O", 1);
					
				}
				if (split[1].indexOf('F') != -1) {
					NonVerb nv = new NonVerb(split[0], split[1], bh, split[0], Integer.parseInt(split[2]));
					nv.deriveGender();
				}
				if (split[1].indexOf('J') != -1 || split[1].indexOf('N') != -1) {
					NonVerb nv = new NonVerb(split[0], split[1], bh, split[0], Integer.parseInt(split[2]));
					nv.derivePlural();
				}
		...
		JOptionPane.showMessageDialog(frame, "La generación de la base de datos ha finalizado con éxito.", "Terminado",
			...
			if (files[i].length() > LIMIT) {
				...
				for (int j = 0; j < ordered.length; j++) {
					...
					if ((bytes > LIMIT && !s.equals(lastRhyme)) || j == ordered.length - 1) {
						...
						String filename = files[i].getName().substring(0, files[i].getName().lastIndexOf('.')) + "_"
								+ count + ".txt";
